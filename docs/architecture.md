...
- В Docker-режиме один экземпляр Harness обслуживает одну рабочую копию, смонтированную как `/workspace`; конфигурация контейнера принудительно ограничивает регистрацию проекта этим путём.
- Изменяющий файловый сценарий не получает shell: модель запрашивает одну из структурированных операций поиска, чтения или записи, а файловый адаптер проверяет путь, тип файла, лимит размера и отсутствие символьных ссылок перед исполнением.
...
Сценарий `testGeneration` создаёт сохраняемый план покрытия и за один запуск обрабатывает ровно один класс. Для MVP анализ ограничен Kotlin-реализациями репозиториев. Изменение происходит только после `fileModification` в изолированном worktree внутри подключённого проекта; модель возвращает содержимое одного заранее разрешённого test-файла и не запускает shell. После успешной фиксированной Gradle-проверки публикация ветки и GitHub PR требует отдельного `externalPublication`. Подробности и конфигурация — в [документации сценария](test-generation.md).
